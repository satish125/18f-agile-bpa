...
function productNameParser($text, $exclusionWords) {
...
function productUpcParser($text, $exclusionWords) {
...
function openFDAProductMatch($type, $days, $minMatchingScore, $minQualityScore) {
    $response = new restResponse;
    ...
    try{
        ...
        // Build array of search terms for product name, filter out common words and special characters
        $productNamePieces = productNameParser($productName, $exclusionWords);  

        // Remove all hyphens in the product upc and convert to lower case
        $productUpc = str_replace('-', '', strtolower($productUpc));

        // Build array of search terms for product name, filter out common words and special characters
        $productUpcPieces = productUpcParser($productUpc, $exclusionWords);
        ...
        foreach ($bigArr['results'] as $idx => &$idxVal) {
            ...
            if ( !array_key_exists('product_description', $idxVal) ) {
                ...
                $resultProductNamePieces = productNameParser($resultProductName, $exclusionWords);
            }
            ...
            if (!array_key_exists('code_info', $idxVal)) {
                ...
                $resultProductUpcPieces = productUpcParser($resultProductUpc, $exclusionWords);               
            }
